...
调用cv2.xfeatures2d.SIFT进行特征点检测、特征匹配
...
def PointCoordinates(eight_points, keypoints1, keypoints2):
    ...
    for i in eight_points:
        tuple_x1 = keypoints1[i[0].queryIdx].pt + tuple_dim#queryIdx特征点序列是图片A中的
        tuple_x2 = keypoints2[i[0].trainIdx].pt + tuple_dim#trainIdx特征点序列是图片B中的
        x1.append(tuple_x1)
...
if __name__ == '__main__':
    ...
    # 进行比值测试，首先获取与 A距离最近的点 B （最近）和 C （次近），只有当 B/C 小于阀值时（0.75）才被认为是匹配，
    # 由于假设匹配是一一对应的，真正的匹配的理想距离为0.第一个m匹配的是最近邻，第二个n匹配的是次近邻。直觉上，一个正确的匹配会更接近第一个邻居
    good = []
    for m, n in matches:
        if m.distance < 0.75 * n.distance:
            good.append([m])
    #good列表中即为两幅图像中的特征点点对
    #使用RANSAC滤除离群点
    ...
    img3 = cv2.drawMatchesKnn(psd_img_1, kp1, psd_img_2, kp2, good, None,[0,255,0], flags=2)
    img4 = cv2.drawMatchesKnn(psd_img_1, kp1, psd_img_2, kp2, inlier_points, None,[0,255,0], flags=2)
    ## 获取关键点的坐标
    tp1 = np.float32([kp1[m[0].queryIdx].pt
                            for m in good]).reshape(-1, 1, 2)
    tp2 = np.float32([kp2[m[0].trainIdx].pt
                            for m in good]).reshape(-1, 1, 2) 
    #根据对应点计算单应性矩阵（图B到图A）,返回值中M为变换矩阵。mask是掩模,参数cv2.RANSAC是使用RANSAC算法寻找一个最佳单应性矩阵H，即返回值M
    M, mask = cv2.findHomography(tp2, tp1, cv2.RANSAC, 5.0)
    ...
    cv2.imwrite(r'./output/SIFT_match.png',img3)
    cv2.imshow("image2", img4)
    cv2.imwrite(r'./output/SIFT_ransac.png',img4)
    cv2.imshow('result', wrap)
    cv2.imwrite(r'./output/SIFT_stitch.png',wrap)
